...
class ShioajiFlowMonitor:
    ...
    def check_flow_limit(self, threshold_percentage: float = 85.0) -> bool:
        """檢查是否接近流量限制"""
        usage_status = self.get_usage_status()
        ...
        try:
            for i, symbol in enumerate(symbols):
                ...
                if self.flow_monitor:
                    ...
                    # 檢查流量限制
                    if usage_info and usage_info.get('percentage', 0) >= 85.0:
                        logger.warning("🚨 流量使用率已達 85%，停止收集以避免超限")
                        break
                ...
                if self.flow_monitor:
                    ...
                    # 如果流量使用率超過 80%，增加休息時間
                    if usage_info and usage_info.get('percentage', 0) > 80:
                        logger.warning("⚠️ 流量使用率超過 80%，增加休息時間")
                        time.sleep(5)
